...
fn bench_sse41_vs_avx2(c: &mut Criterion) {
    ...
    let test_cases = vec![
        ...
        ("long", "this_is_a_longer_string_for_testing", "this_is_another_longer_string_test"),
    ];

    for (name, source, target) in test_cases {
        group.bench_with_input(BenchmarkId::new("simd", name), &(source, target), |b, (s, t)| {
            b.iter(|| standard_distance_simd(black_box(s), black_box(t)))
        });

        group.bench_with_input(BenchmarkId::new("scalar", name), &(source, target), |b, (s, t)| {
            b.iter(|| standard_distance_impl(black_box(s), black_box(t)))
        });
    }
...
fn bench_affix_stripping(c: &mut Criterion) {
    ...
    let test_cases = vec![
        ...
        ("common_prefix_16", "long_prefix_here_abc", "long_prefix_here_xyz"),
    ...
    for (name, source, target) in test_cases {
        group.bench_with_input(BenchmarkId::new("simd", name), &(source, target), |b, (s, t)| {
            b.iter(|| strip_common_affixes_simd(black_box(s), black_box(t)))
        });

        group.bench_with_input(BenchmarkId::new("scalar", name), &(source, target), |b, (s, t)| {
            b.iter(|| liblevenshtein::distance::strip_common_affixes(black_box(s), black_box(t)))
        });
    }
...
fn bench_integrated_distance(c: &mut Criterion) {
    ...
    let test_cases = vec![
        ...
        ("long_prefix", "common_prefix_different_ending_abc", "common_prefix_different_ending_xyz"),
        ("long_identical", "this_is_identical_string", "this_is_identical_string"),
    ];

    for (name, source, target) in test_cases {
        group.bench_with_input(BenchmarkId::new("optimized", name), &(source, target), |b, (s, t)| {
