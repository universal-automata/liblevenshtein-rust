...
This theorem establishes that the naive O(n·m) dynamic programming implementation of Levenshtein distance correctly computes the minimum edit distance between two strings, matching the formal Wagner-Fischer recurrence relation.
...
**Therefore**: DP matrix computes exact same values as recursive definition. ∎
...
6. Therefore: `lev_distance s1 s3 ≤ |e12 ++ e23| = lev_distance s1 s2 + lev_distance s2 s3` ∎
...
For any edit sequence `s1 → s2`, the reversed sequence of inverse operations is `s2 → s1` with the same length. ∎
...
- **Inductive step**:
  ...
  - Then `lev_distance (c::rest) (c::rest) = min3(lev_distance rest (c::rest) + 1, lev_distance (c::rest) rest + 1, lev_distance rest rest + 0) = min3(IH+1, IH+1, 0) = 0` ∎

### Part 5: Non-Negativity

**Claim**: `lev_distance s1 s2 ≥ 0`

**Proof**: By definition, `lev_distance` returns a `nat` (natural number), which is always ≥ 0. ∎
...
- This gives: `lev_distance s1 s2 ≤ max(length s1, length s2)` ∎
...
- **Space**: O(n·m) for matrix (could be optimized to O(min(n, m)) using rolling arrays)
