...
impl CommandHighlighter {
    pub fn new() -> Self {
        Self {
            commands: vec![
                "query", "q", "insert", "add", "delete", "remove", "rm",
                "contains", "has", "load", "save", "backend", "use",
                "algorithm", "algo", "distance", "dist", "prefix",
                "show-distances", "show-dist", "limit", "clear", "compact",
                "minimize", "dump", "list", "stats", "info", "settings",
                "config", "set", "help", "?", "exit", "quit",
            ]
            .into_iter()
            .map(String::from)
            .collect(),
