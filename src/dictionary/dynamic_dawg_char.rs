...
#[cfg(feature = "serialization")]
impl<'de, V: DictionaryValue + serde::Deserialize<'de>> serde::Deserialize<'de> for DynamicDawgChar<V> {
...
impl<V: DictionaryValue> DictionaryNode for DynamicDawgCharNode<V> {
    ...
    fn edges(&self) -> Box<dyn Iterator<Item = (char, Self)> + '_> {
        ...
        Box::new(child_data.into_iter().map(move |(ch, idx, is_final, edges)| {
            (
                ch,
                DynamicDawgCharNode {
                    dawg: Arc::clone(&dawg),
...
mod tests {
    ...
    fn test_dynamic_dawg_with_transducer() {
        ...
        let dawg: DynamicDawgChar<()> = DynamicDawgChar::from_terms(vec!["apple", "application", "apply"]);
    ...
    fn test_batch_remove_many() {
        let dawg: DynamicDawgChar<()> = DynamicDawgChar::from_terms(vec!["test", "testing", "tested", "tester"]);
    ...
    fn test_minimize_idempotent() {
        let dawg: DynamicDawgChar<()> = DynamicDawgChar::from_terms(vec!["apple", "application", "apply", "apricot"]);
