...
fn check_transposition_subsumption_both_ways() {
    ...
    println!("p_normal:  ({}, {}, false)", p_normal.term_index, p_normal.num_errors);
    println!("p_special: ({}, {}, true)", p_special.term_index, p_special.num_errors);
    println!();

    // Check normal subsumes special
    let normal_subsumes_special = p_normal.subsumes(&p_special, algorithm, query_length);
    println!("Does p_normal subsume p_special? {}", normal_subsumes_special);

    // Check special subsumes normal
    let special_subsumes_normal = p_special.subsumes(&p_normal, algorithm, query_length);
    println!("Does p_special subsume p_normal? {}", special_subsumes_normal);
    println!();
    ...
    if special_subsumes_normal {
        ...
        println!("    f={}, query_length={}, i={}, j={}", p_normal.num_errors, query_length, p_special.term_index, p_normal.term_index);
        println!("    Condition: (f == query_length) && (i == j)");
        println!("    = ({} == {}) && ({} == {})", p_normal.num_errors, query_length, p_special.term_index, p_normal.term_index);
        println!("    = (1 == 2) && (0 == 0)");
        println!("    = false && true");
        println!("    = false");
        println!();
