...
fn trace_aple_to_apple() {
    ...
    state = transition_state(&state, b'a', query, max_distance, Algorithm::Standard)
    ...
    state = transition_state(&state, b'p', query, max_distance, Algorithm::Standard)
    ...
    if let Some(p2_node) = p1_node.transition(b'p') {
        ...
        if let Some(new_state) = transition_state(&state, b'p', query, max_distance, Algorithm::Standard) {
            state = new_state;
            ...
            if let Some(l_node) = p2_node.transition(b'l') {
                ...
                if let Some(new_state) = transition_state(&state, b'l', query, max_distance, Algorithm::Standard) {
                    state = new_state;
                    ...
                    if let Some(e_node) = l_node.transition(b'e') {
                        ...
                        if let Some(new_state) = transition_state(&state, b'e', query, max_distance, Algorithm::Standard) {
                            state = new_state;
