...
fn main() {
    ...
    let query_words: Vec<_> = words.iter()
    ...
    println!("  µs/query: {:.2}", time_generic.as_micros() as f64 / query_words.len() as f64);
    println!("  Results: {}\n", results_generic);
    ...
    println!("  µs/query: {:.2}", time_optimized.as_micros() as f64 / query_words.len() as f64);
    println!("  Results: {}\n", results_optimized);
    ...
    println!("✅ Both iterators return identical results ({} matches)\n", results_generic);
    ...
    let improvement = ((time_generic.as_nanos() - time_optimized.as_nanos()) as f64
        / time_generic.as_nanos() as f64) * 100.0;

    println!("Performance Improvement:");
    println!("  Generic: {:.2} µs/query", time_generic.as_micros() as f64 / query_words.len() as f64);
    println!("  Optimized: {:.2} µs/query", time_optimized.as_micros() as f64 / query_words.len() as f64);
    ...
    for distance in [1, 2, 3] {
        ...
        println!("  Generic: {:.2} µs/query, Optimized: {:.2} µs/query, Speedup: {:.2}x",
